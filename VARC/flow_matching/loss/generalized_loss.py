...
class MixturePathGeneralizedKL(_Loss):
    r"""Generalized KL for discrete flow matching with mixture probability paths."""
    ...
    def forward(self, logits: Tensor, x_1: Tensor, x_t: Tensor, t: Tensor) -> Tensor:
        ...
        log_p_1t_x1 = torch.gather(log_p_1t, dim=-1, index=x_1.unsqueeze(-1)).view(*x_1_shape)

        p_1t = torch.exp(log_p_1t)
        p_1t_xt = torch.gather(p_1t, dim=-1, index=x_t.unsqueeze(-1)).view(*x_1_shape)

        scheduler_output = self.path.scheduler(t)
        jump_coefficient = scheduler_output.d_alpha_t / (1.0 - scheduler_output.alpha_t).clamp_min(1e-12)
        while jump_coefficient.dim() < x_1.dim():
            jump_coefficient = jump_coefficient.unsqueeze(-1)
